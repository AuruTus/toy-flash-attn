...
from tests.utils import BATCH_SIZE_FOR_SEQ_LEN, BENCHMARK_N_HEADS  # noqa: E402
...
def parse_ncu_csv_rows(csv_reader):
    ...
    for row in csv_reader:
        ...
        if "flash_fwd_kernel" in kernel_name:
            ...
            kernel_name = parse_kernel_name_into_config(kernel_name).short_form()
        ...
        for user_metric, info in METRICS_MAP.items():
            if metric_name == info["csv_name"]:
                kernels[kernel_name][user_metric] = float(metric_value.replace(",", ""))
                break
...
def generate_results_table(
    ...
    ref_v2_item = (REFERENCE_V2, kernels[REFERENCE_V2]) if REFERENCE_V2 in kernels else None
    ref_v3_item = (REFERENCE_V3, kernels[REFERENCE_V3]) if REFERENCE_V3 in kernels else None
    other_items = [(k, v) for k, v in kernels.items() if k not in REFERENCE_KERNELS]
    ...
    for kernel_name, metrics in kernels_sorted:
        ...
        if kernel_name in REFERENCE_KERNELS:
            ...
            flop = calc_total_flop(
                BATCH_SIZE_FOR_SEQ_LEN[seq_len],
                BENCHMARK_N_HEADS,
                seq_len,
                B_r,
                B_c,
            ...
            flop = parse_kernel_name_into_config(kernel_name).total_flop(
                BATCH_SIZE_FOR_SEQ_LEN[seq_len], BENCHMARK_N_HEADS, seq_len
            )

        tflops = flop / duration_ns / 1e3
        attn_flops = (
            calc_self_attn_flop(BATCH_SIZE_FOR_SEQ_LEN[seq_len], BENCHMARK_N_HEADS, seq_len, d_head) / duration_ns / 1e3
        )
        row.append(f"{tflops}")
...
def call_ncu_and_store_output(program: list[str]):
    ...
    try:
        ...
        print("The `ncu` command was not found. Ensure it's installed and in your PATH.")
        return {}
...
def parse_cmd_args():
    parser = argparse.ArgumentParser(description="Parse d_heads and seq_lens arguments, plus number of runs.")
    parser.add_argument(
        "--d_heads",
        type=str,
        default="128",
    ...
    parser.add_argument("--csv", action="store_true", dest="csv", help="Output as csv")
    parser.add_argument(
        "--no_sort",
        action="store_true",
        dest="no_sort",
...
def get_highest_profile_number(directory):
    ...
    for file in directory.iterdir():
        match = re.match(r"profile_(\d+)\.", file.name)
        if match:
...
PROFILE_DIR = PROJECT_DIR / "profiles" / "local_profiles"
...
if __name__ == "__main__":
    ...
    for d_head, seq_len in itertools.product(d_heads, seq_lens):
        ...
        # Run multiple times
        for _ in range(runs):
            run_metrics = call_ncu_and_store_output(PROGRAM + [seq_len, d_head, "--kernels", "all"])
        ...
        table = generate_results_table(averaged_results, int(d_head), int(seq_len), print_csv, sort=sort)
        if not print_csv:
            logging.info(table)
            logging.info("")
        else:
