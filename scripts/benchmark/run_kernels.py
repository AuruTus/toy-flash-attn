...
from toy_attn.flash_attn_v2 import flash_attention  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Run all flash attention kernels once")
    parser.add_argument("seq_len", type=int, nargs="?", default=4096, help="Sequence length")
    parser.add_argument("d_head", type=int, nargs="?", default=128, help="Head dimension")
    parser.add_argument("--ref", action="store_true", help="Run reference kernels")
    parser.add_argument("--ref_v3", action="store_true", help="Run reference v3 kernel")
    parser.add_argument("--n_runs", type=int, default=1, help="Run kernels N times (default: 1)")
    parser.add_argument(
        "--kernels",
        nargs="+",
    ...
    if args.kernels is not None:
        ...
        if len(args.kernels) == 1 and args.kernels[0] == "all":
            selected_kernels = get_kernel_configs("all")
        else:
    ...
    for cfg in selected_kernels:
        ...
        for _ in range(args.n_runs):
            _ = flash_attention.forward(cfg, tensors["q"], tensors["k"], tensors["v"], o)
            torch.cuda.synchronize()
