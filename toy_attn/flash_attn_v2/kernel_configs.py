...
class DType(IntEnum):
    ...
    def from_string(cls, dtype_str: str) -> "DType":
        ...
        try:
            ...
            valid_options = [f"{member.name} ({member.value})" for member in cls]
            raise ValueError(f"Invalid dtype string '{dtype_str}'. Valid options: {valid_options}")
...
def calc_total_flop(n_samples: int, n_heads: int, seq_len: int, B_r: int, B_c: int, d_head: int) -> int:
    ...
    head_sample_flops = T_r * (T_c * kv_tile_flop(B_r, B_c, d_head) + epilogue_flops)
...
class FlashForwardKernelConfig:
    ...
    def short_form(self, include_d_head: bool = True, include_tup: bool = True) -> str:
        ...
        strs.append(f"load_{self.Q_mma_load_K_tiles}_{self.K_mma_load_K_tiles}_{self.V_mma_load_K_tiles}_tiles")
    ...
    def total_flop(self, n_samples: int, n_heads: int, seq_len: int) -> int:
        return calc_total_flop(n_samples, n_heads, seq_len, self.B_r, self.B_c, self.d_head)
...
def _parse_flash_forward_demanged_name_with_types(
    ...
    if not match:
        raise ValueError("Invalid line format: FlashForwardKernelConfig block not found")
...
def _parse_short_form_flash_forward_kernel_config(
    ...
    if not match:
        raise ValueError(f"Cannot parse config string (no matching pattern): {config_str}")
    ...
    if not load_segment:
        raise ValueError(f"Cannot find load segment in features: {features_str}")
...
def should_autotune_config(cfg: FlashForwardKernelConfig) -> bool:
    if not cfg.async_copy and cfg.eager_load_blocks:
        return False
    if cfg.Q_mma_load_K_tiles != cfg.K_mma_load_K_tiles and cfg.Q_mma_load_K_tiles != 0:
        return False

    if cfg.B_r == 64:
        if cfg.n_warps == 8:
            return False
        elif cfg.B_c == 32 and cfg.Q_mma_load_K_tiles == 0:  # over threshold of # registers for 3 CTA
            return False
...
def get_autotuning_kernel_configs(dtypes: list[DType] = [DType.BF16, DType.FP16]) -> list[FlashForwardKernelConfig]:
    ...
    eager_load_blocks = [True, False]
    swizzleds = [True, False]
...
def get_kernel_progression_configs(all_block_sizes: bool = False) -> list[FlashForwardKernelConfig]:
    ...
    base_progression = [_parse_short_form_flash_forward_kernel_config(cfg) for cfg in base_progression]
    if not all_block_sizes:
        return base_progression

    kernels = []
    for kernel in base_progression:
        for B_r, B_c, n_warps in itertools.product([128, 64], [128, 64, 32], [4, 8]):
...
def get_kernel_configs(kernels_key: str = "") -> list[FlashForwardKernelConfig]:
    ...
    if kernels_key.startswith("prog"):
        return get_kernel_progression_configs(all_block_sizes="all" in kernels_key)
        ...
        return [cfg for cfg in autotune_cfgs if cfg.B_r == B_r and cfg.B_c == B_c]
    else:
