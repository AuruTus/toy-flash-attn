...
__forceinline__ __device__ constexpr void calc_row_max(
    ...
    for (int q = 0; q < QO_fragments; ++q) {
        ...
        m_next[q] =
            max(__shfl_xor_sync(SHFL_ENTIRE_WARP_MASK, m_next[q], 2),
                m_next[q]);
        m_next[q] =
            max(__shfl_xor_sync(SHFL_ENTIRE_WARP_MASK, m_next[q], 1),
                m_next[q]);
    }
}
...
__forceinline__ __device__ constexpr void scale_l_O(
    ...
    for (int q = 0; q < QO_fragments; ++q) {
        ...
        if constexpr (optimized_softmax) {
            scale = exp2f((m_curr[q] - m_next[q]) * softmax_scale);
        } else {
