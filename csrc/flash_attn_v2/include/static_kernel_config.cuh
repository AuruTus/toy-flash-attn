...
constexpr bool valid_config() {
    static_assert_valid_load_k_fragments<
        CFG.Q_mma_load_K_fragments, CFG.d_head, CFG.mma_double_buffer_loads
    >();
    static_assert_valid_load_k_fragments<
        CFG.K_mma_load_K_fragments, CFG.d_head, CFG.mma_double_buffer_loads
    >();
    static_assert_valid_load_k_fragments<
        CFG.V_mma_load_K_fragments, CFG.B_c, CFG.mma_double_buffer_loads
    >();

    static_assert(
        (CFG.Q_mma_load_K_fragments == CFG.K_mma_load_K_fragments) ||
...
struct StaticForwardKernelConfig {
    using accum_t = float;
    using value_t = typename std::
        conditional_t<CFG.dtype == torch::kBFloat16, nv_bfloat16, half>;
    ...
    using O_PV_GEMM = GEMM<
        P_value_t,
        V_t,
        O_accum_t,
        N::KV_calc_fragments,
        N::V_mma_load_K_fragments,
        value_t
    >;
