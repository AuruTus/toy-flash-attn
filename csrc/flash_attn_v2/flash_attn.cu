...
auto flash_attention_forward(
    ...
    TORCH_CHECK(
        TK.dtype() == Q_dtype, "Input tensors must have the same data type"
    );
    TORCH_CHECK(
        TV.dtype() == Q_dtype, "Input tensors must have the same data type"
    );

    const auto d_head = TQ.size(3);
    const FlashForwardKernelConfig cfg{py_to_cpp_kernel_config(py_cfg)};
    TORCH_CHECK(
        forward_kernels.contains(cfg),
    ...
    TORCH_CHECK(
        TQ.sizes() == TK.sizes(), "Query and key tensors have same shape"
    );
    TORCH_CHECK(
        TQ.sizes() == TV.sizes(), "Query and value tensors have same shape"
    );
    ...
    if (out_.has_value()) {
        ...
        TORCH_CHECK(
            TQ.sizes() == TV.sizes(), "Query and output tensors have same shape"
        );
    } else {
    ...
    const int n_threads   = cfg.n_warps * WARP_SIZE_DEFAULT;
    ...
    dim3 gridDim{
        static_cast<uint>(n_Q_blocks), static_cast<uint>(n_heads),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        "forward", &flash_attention_forward, py::arg("kernel_cfg"),
