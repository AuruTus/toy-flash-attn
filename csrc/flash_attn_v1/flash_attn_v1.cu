...
__global__ void flash_attn_kernel_v1(
    ...
    // paralleled kernel Q K V stride {nh * Tc * Bc * d, Tc * Bc * d, Bc * d, d,
    // 1}
    ...
    for (int j = 0; j < Tc; ++j) {
        for (int x = 0; x < d; ++x) {
            // a Block owns Bc threads, and each one copy a head-dim K, V vector
            // into smem
        ...
        for (int i = 0; i < Tr; ++i) {
            for (int x = 0; x < d; ++x) {
                ...
                Q_i[(tx * d) + x] =
                    Q[qkv_offset + (i * TILE_SIZE) + (tx * d) + x];
            }
            ...
            scalar_t row_l_new = (__expf(row_m_prev - row_m_new) * row_l_prev) +
...
torch::Tensor
toy_flash_attn_cuda(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        Q.scalar_type(), "toy_flash_attn_cuda", ([&] {
            scalar_t softmax_scale = 1.0f / sqrtf(static_cast<scalar_t>(d));

            // share mem with [Q {Bc, d}, K {Bc, d}, V {Bc, d}, S {Bc, Bc}]
            int smem_size =
                (3 * Bc * d * sizeof(scalar_t)) + (Bc * Bc * sizeof(scalar_t));

            // launch B * nh blocks
            dim3 grid_dim(B, nh);
            // each block has Bc threads, and handle sub Q, K V chunk with {Tc,
            // Bc, d} and {Tr, Br, d} respectively
            dim3 block_dim(Bc);

            int max_sram_size;
            cudaDeviceGetAttribute(
                &max_sram_size, cudaDevAttrMaxSharedMemoryPerBlock, 0
            );
            flash_attn_kernel_v1<<<grid_dim, block_dim, smem_size>>>(
                Q.data_ptr<scalar_t>(), K.data_ptr<scalar_t>(),
                V.data_ptr<scalar_t>(), N, d, Tc, Tr, Bc, Br, softmax_scale,
                l.data_ptr<scalar_t>(), m.data_ptr<scalar_t>(),
                O.data_ptr<scalar_t>()
            );
        })
    );
